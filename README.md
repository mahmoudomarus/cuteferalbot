...
**Wiring this into another system?** Read **[INTEGRATION.md](INTEGRATION.md)** — the single-source integrator's reference: install, the three integration tiers (raw / closed-loop / LLM), full API tables, JSON schemas, error model, threading, and a smoke checklist.
...
For higher-level "smart" behaviors — overhead-camera localization,
closed-loop go_to/patrol, real-time YOLO awareness, voice control, and a
local LLM tool-calling agent — see the [`brain/`](brain/README.md)
package. It runs entirely on the Mac and stacks on top of `QtBot` without
...
# optional: brain extras (perception + LLM agent + voice)
...
firmware/
  ...
  autonomous.py    AutoBrain: Case 07/08/09 behaviors + tilt safety + proportional steering
...
brain/                          optional Mac-side AI layer (see brain/README.md)
  perception/
    camera.py        shared CameraStream
    localize.py      ArUco overhead pose -> (x_cm, y_cm, heading_deg)
    vision.py        YOLO detector + world-frame projection + optional VLM
    voice.py         local STT (parakeet/whisper) + TTS
  navigation.py      closed-loop go_to / patrol controller
  tools.py           Toolbelt: JSON tool-call surface for the agent
  cognition/agent.py Ollama tool-calling LLM loop
